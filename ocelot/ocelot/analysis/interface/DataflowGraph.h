	...
	*/
	class DataflowGraph
	{
		...
		public:			
			...
			{
				public:
					unsigned int operator()(RegisterVector::const_iterator it)
					{
						return it->id;
					}
			};
			...
			*/
			class Block
			{
				...
				public:
					...
					/*! \brief Get an ordered (and modifiable) set of phis in
					 * the block*/
		...
		public:
			/*!-------------- Basic block and edge manipulation -------------*/
			...
			*/
			...
			iterator split( iterator block, unsigned int instruction, 
				bool isFallthrough );
			/*! \brief Add a branch edge from source basic block to target */
			void addEdge( iterator source, iterator target, ir::ControlFlowGraph::Edge::Type type );
			/*! \brief Redirect an edge between two blocks to a third */
			void redirect( iterator source, 
				iterator destination, iterator newTarget );
			/*! \brief Copy edges from source to destination block */
			void copyOutgoingBranchEdges( iterator source, iterator destination );
			/*! \brief Set the target of a block */
			void target( iterator block, iterator target );
			/*! \brief Change a edge type */
			void setEdgeType( iterator source, iterator destination, ir::ControlFlowGraph::Edge::Type type );
			/*! \brief Delete a block, joining predecessors and successors */
			iterator erase( iterator block );
			/*! \brief Remove a edge from source basic block to target.
			    \return True if successfully removed such edge and false
			    otherwise. */
			bool removeEdge( iterator source, iterator target );
			/*! \brief Revert back to a single entry and exit block */
			void clear();
		
		public:
			/*!-------- Instruction manipulation inside basic blocks --------*/
		...
		public:
			...
			void updateMaxRegister(RegisterId newMaxREgister)
			{
				_maxRegister = newMaxREgister;
			}
		...
		public:
			...
			std::ostream& writeCfgPlusPhis(std::ostream &out) const;
