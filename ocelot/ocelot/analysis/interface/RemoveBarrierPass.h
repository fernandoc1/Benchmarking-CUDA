...
namespace analysis
{
	...
	/*! \brief A class for a pass that removes all barriers from a PTX kernel
		...
		This implementation identifies barriers and splits the basic block
		containing them into two. The first block contains all of the code
		before the barrier, spill instructions to a stack in local memory,
		and a branch to the program exit. A local variable is allocated on the
		stack to indicate the program entry point.
		
		The program entry point is augmented to include a conditional branch to
