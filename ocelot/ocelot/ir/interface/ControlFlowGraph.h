...
public:
	...
	/*!	Adds an instruction at the beginning of the basic block. */
	void addInstructionBegin(Instruction* i) {
		instructions.push_front(i);
	}

	/*! Adds an instruction *before* the first non-terminator instruction
		in the basic block. A terminator is any instruction that ends
		the basic block. Currently we are considering three terminators:
		branches, returns and exists. */
	void addInstructionEnd(Instruction* i);

	/*! Adds an instruction before the iterator it.
		\param it the anchoring position. The instruction will be inserted before it. */
	void addInstruction(InstructionList::iterator& it, Instruction* i) {
		// insert the new instruction in block instruction list
		instructions.insert(it, i);
	}

	/*! Return true if the block finishes with a conditional branch. */
	bool endsWithConditionalBranch() const;

	/*! Return true if the block ends with a unconditional branch. */
	bool endsWithUnconditionalBranch() const;

	/*! If the block isn't empty, return its last instruction (terminator),
		otherwise return NULL. */
	Instruction* getTerminator() const;

	/*! Sets the label of a basic block */
