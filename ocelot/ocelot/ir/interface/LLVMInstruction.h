...
namespace ir
{
	...
	class LLVMInstruction : public Instruction
	{
		...
		public:
			...
			const LLVMInstruction::Opcode getOpcode() const {
				return this->opcode;
			}
			
			/*! Verify if the instruction is a basic block terminator */
			bool isTerminator() const {
				LLVMInstruction::Opcode op = getOpcode();
				
				switch (op) {
				    case ir::LLVMInstruction::Br:	// branch - jumps to another basic block.
				    case ir::LLVMInstruction::Ret:	// return - ends the function
					    return true;
				    default:
				        return false;
				}
			...
			/*! Verify if the instruction is a unconditional branch */
