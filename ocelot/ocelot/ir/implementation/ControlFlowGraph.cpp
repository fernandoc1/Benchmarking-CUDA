...
void ControlFlowGraph::BasicBlock::addInstructionEnd(Instruction* newInst) {

	// We cannot simply insert instIdx at the end of the basic block.
	// What we really want is to insert instIdx *after* the first non-
	// terminator in the basic block. A terminator is an instruction that
	// ends the basic block, such as a branch, a return or an exit.
	InstructionList::const_iterator begin = instructions.begin();
	InstructionList::iterator end         = instructions.end();

	if (begin == end) {
		return addInstructionBegin(newInst);
	}

	InstructionList::iterator it = end;
	it--;
    ...
    // Re-increment it, because the insert method inserts *before*,
    // not after the iterator, and we want to insert instIdx before
    // the first terminator not before the last non-terminator.
    Instruction* inst = *it;
    if (!inst->isTerminator()) {
    	it++;
    }

    // insert the new instruction into block instruction list
	instructions.insert(it, newInst);
}

bool ControlFlowGraph::BasicBlock::endsWithConditionalBranch() const {
	const Instruction* terminator = this->getTerminator();

	if (terminator) {
		// block has a terminator (bra, ret, exit)
		if (terminator->isConditionalBranch()) {
			return true;
		}
	}
...
bool ControlFlowGraph::BasicBlock::endsWithUnconditionalBranch() const {
	const Instruction* terminator = this->getTerminator();
...
Instruction* ControlFlowGraph::BasicBlock::getTerminator() const
{
	InstructionList::const_iterator begin = instructions.begin();
	...
	InstructionList::const_iterator it = end;
	it--;
	Instruction* inst = *it;
