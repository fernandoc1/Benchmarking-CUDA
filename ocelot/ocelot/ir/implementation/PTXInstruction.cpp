...
const char* ir::OperandTypeException::what() const throw()
{
	std::string cause;

	if (instruction != NULL) {
		cause = "Operand type " + PTXOperand::toString(operandType)
			+ " isn't compatible with instruction " + instruction->toString();
			//+ "(incomplete instruction, some operands may be missing)";
	} else {
		cause = "Operand type " + PTXOperand::toString(operandType) +
			" isn't compatible with instruction type " + PTXOperand::toString(instructionType);
	}
...
ir::PTXInstruction::PTXInstruction(Opcode op) 
	: opcode(op)
{
	ISA = Instruction::PTX;
...
std::string ir::PTXInstruction::printInst() const
{
	return toString();
}
...
const ir::PTXOperand* ir::PTXInstruction::getConstOperand(const ir::PTXInstruction::Operand op) const
{
	const ir::PTXOperand* ret = NULL;

	switch (op) {
	case ir::PTXInstruction::PredicateGuard:
		ret = &pg;
		break;
	case ir::PTXInstruction::OperandA:
		ret = &a;
		break;
	case ir::PTXInstruction::OperandB:
		ret = &b;
		break;
	case ir::PTXInstruction::OperandC:
		ret = &c;
		break;
	case ir::PTXInstruction::OperandD:
		ret = &d;
		break;
	case ir::PTXInstruction::OperandQ:
		ret = &pq;
		break;
	default:
		assertM(false, "getOperand(): Unhandled operand\n");
		break;
	}
...
/*
 * The main complexity of this method steams from the fact that the type of
 * the instruction may change as soon as we modify one of its operands.
 * For instance, the cons method of the instruction initializes it with the
 * type 'invalid'. When we add a new operand to the instruction, we see if its
 * type is still 'invalid'. In this case, we change the type of the instruction
 * to reflect the type of the operand; otherwise, we do type verification to
 * find out if we have compatible types, that is, if that operand fits in the
 * destination. If we have a case of type mismatching, then we throw an
 * OperandTypeException.
 */
void ir::PTXInstruction::setDestination(const ir::PTXOperand& dest) {
		
	// There are special instructions that can have the destination type
	// different than the source type. We must handle these cases:
	switch (opcode) {
		case PTXInstruction::Bra:
			// the Bra destination operand is a label, that has no type  
			break;
			
		case PTXInstruction::Mad:
		case PTXInstruction::Mul:
			if (this->modifier == PTXInstruction::wide) {
				PTXOperand::DataType newInstType = ir::PTXOperand::wideToShort(dest.type);
				if (newInstType == ir::PTXOperand::TypeSpecifier_invalid) {
					throw OperandTypeException(this, dest.type, type);
				}

				checkOperandType(newInstType);
			} else {
				checkOperandType(dest.type);
			}
			break;
			
		case PTXInstruction::SetP:
			// the type of the SetP instruction must be the type of the a and b operands 
			break;
...
void ir::PTXInstruction::setOperandA(const ir::PTXOperand& opr) {
		
	// There are special instructions that can have the destination type
	// different than the source type. We must handle these cases:
	switch (opcode) {
		case PTXInstruction::Cvt:
			// the type of the cvt instruction must always be equal to the
			// type of the destionation operand. 
			break;
		default:
			checkOperandType(opr.type);
			break;
	}
...
void ir::PTXInstruction::setOperandB(const ir::PTXOperand& opr) {
	checkOperandType(opr.type);
	b = opr;	
}

/* This method is fairly more complex than the other set operand methods.
 * This extra complexity is due to the 'mad' instruction in the wide mode. An
 * instruction such as D = A * B + C requires the type of C being twice the
 * size of the types of the operands A and B.
 */
void ir::PTXInstruction::setOperandC(const ir::PTXOperand& opr) {
	
	switch (opcode) {
		case Fma:
		case Mad:
			if (this->modifier == PTXInstruction::wide) {
				PTXOperand::DataType newInstType = ir::PTXOperand::wideToShort(opr.type);
				if (newInstType == ir::PTXOperand::TypeSpecifier_invalid) {
					throw OperandTypeException(this, opr.type, type);
				}
			...
			break;
		case Mad24:
			// only u32 and s32 allowed
			if ((opr.type != PTXOperand::u32) && (opr.type != PTXOperand::s32)) {
				throw OperandTypeException(this, opr.type, type);
			}
			checkOperandType(opr.type);
			break;
		...
		case SetP:
		case SelP:
			// In these instructions, operand C must be a predicate.
			if (opr.type != PTXOperand::pred) {
				throw OperandTypeException(this, opr.type, type);
			}	
			break;
		case SlCt:
			// Operand C can have any type
			break;
		default:
			checkOperandType(opr.type);
			break;
...
ir::PTXInstruction::OperandPresence ir::PTXInstruction::getOperandPresence(const ir::PTXInstruction::Operand operand) const
{
	if (operand == ir::PTXInstruction::PredicateGuard) {
		return ir::PTXInstruction::PresenceOptional;
	}

	if (operand == ir::PTXInstruction::OperandQ) {
		// only present in SetP
		if (opcode == ir::PTXInstruction::SetP) {
			return ir::PTXInstruction::PresenceOptional;
		}
		return ir::PTXInstruction::PresenceDenied;
	}
	...
	// instructions that use only d operand
	...
	case ir::PTXInstruction::Ret:
		if (operand == ir::PTXInstruction::OperandD) {
			return ir::PTXInstruction::PresenceCompulsory;
		}
		return ir::PTXInstruction::PresenceDenied;
		break;

	// instructions with d and a operands
	...
	case ir::PTXInstruction::Vote:
		if ((operand == ir::PTXInstruction::OperandD) ||
				(operand == ir::PTXInstruction::OperandA)) {
			return ir::PTXInstruction::PresenceCompulsory;
		}
		return ir::PTXInstruction::PresenceDenied;
		break;

	// instruction with operands d,a and b
	...
	case ir::PTXInstruction::Xor:
		if (operand == ir::PTXInstruction::OperandC) {
			return ir::PTXInstruction::PresenceDenied;
		}
		return ir::PTXInstruction::PresenceCompulsory;
		break;

	// instructions with operand d,a,b and an optional c operand
	...
	// instruction with all operands: d,a,b,c
	...
	// instructions that use a,b,c operands
	...
	// this instruction uses d,a,c and have a optional b operand
	case ir::PTXInstruction::Tex:
		if (operand == ir::PTXInstruction::OperandB) {
			return ir::PTXInstruction::PresenceOptional;
		}
		return ir::PTXInstruction::PresenceCompulsory;
		break;
	default:
		assertM(false, "getOperandBPresence(): unhandled instruction opcode\n");
		break;
	}

	// just to gcc not complain, control flow not reach here.
	return ir::PTXInstruction::PresenceDenied;
}

bool ir::PTXInstruction::isUsingOperand(const ir::PTXInstruction::Operand op) const
{
	ir::PTXInstruction::OperandPresence p = getOperandPresence(op);

	if (p == ir::PTXInstruction::PresenceDenied) {
		return false;
	}
	if (p == ir::PTXInstruction::PresenceCompulsory) {
		return true;
	}

	// optional operand, check if it's used
	const ir::PTXOperand* optionalOperand = getConstOperand(op);
	assert(optionalOperand != NULL);

	if (optionalOperand->addressMode == ir::PTXOperand::Invalid) {
		return false;
	}
	if ((op == ir::PTXInstruction::PredicateGuard) ||
			(op == ir::PTXInstruction::OperandQ)) {
		if (optionalOperand->condition == ir::PTXOperand::PT) {
			return false;
		}
	}
	return true;
