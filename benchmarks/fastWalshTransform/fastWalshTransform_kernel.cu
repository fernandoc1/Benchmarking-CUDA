...
void fwtBatchGPU(float *d_Data, int M, int log2N){
	...
	GpuProfiling::prepareProfiling( grid, THREAD_N );
        fwtBatch2Kernel<<<grid, THREAD_N>>>(d_Data, d_Data, N / 4);
	GpuProfiling::addResults("fwtBatch2Kernel");
        cutilCheckMsg("fwtBatch2Kernel() execution failed\n");
    }

	GpuProfiling::prepareProfiling( M, N / 4, N * sizeof(float) );
    fwtBatch1Kernel<<<M, N / 4, N * sizeof(float)>>>(
        d_Data,
        d_Data,
        log2N
    );
	GpuProfiling::addResults("fwtBatch1Kernel");
...
void modulateGPU(float *d_A, float *d_B, int N){
	GpuProfiling::prepareProfiling( 128, 256 );
    modulateKernel<<<128, 256>>>(d_A, d_B, N);
	GpuProfiling::addResults("modulateKernel");
}
