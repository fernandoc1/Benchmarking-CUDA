...
extern "C" size_t scanExclusiveShort(
	...
	GpuProfiling::prepareProfiling( (batchSize * arrayLength) / (4 * THREADBLOCK_SIZE), THREADBLOCK_SIZE );
    scanExclusiveShared<<<(batchSize * arrayLength) / (4 * THREADBLOCK_SIZE), THREADBLOCK_SIZE>>>(
        (uint4 *)d_Dst,
        (uint4 *)d_Src,
        arrayLength
    );
	GpuProfiling::addResults("scanExclusiveShared");
...
extern "C" size_t scanExclusiveLarge(
	...
	GpuProfiling::prepareProfiling(  blockCount2, THREADBLOCK_SIZE );
	...
	GpuProfiling::addResults("scanExclusiveShared2");
	...
	GpuProfiling::addResults("uniformUpdate");
