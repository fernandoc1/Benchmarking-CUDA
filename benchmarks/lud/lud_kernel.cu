...
GpuProfiling::initProf();
  ...
  for (i=0; i < matrix_dim-BLOCK_SIZE; i += BLOCK_SIZE) {
	GpuProfiling::prepareProfiling( 1, BLOCK_SIZE );
      lud_diagonal<<<1, BLOCK_SIZE>>>(m, matrix_dim, i);
	GpuProfiling::addResults("lud_diagonal");
	GpuProfiling::prepareProfiling( (matrix_dim-i)/BLOCK_SIZE-1, BLOCK_SIZE*2 );
      lud_perimeter<<<(matrix_dim-i)/BLOCK_SIZE-1, BLOCK_SIZE*2>>>(m, matrix_dim, i);
	GpuProfiling::addResults("lud_perimeter");
      dim3 dimGrid((matrix_dim-i)/BLOCK_SIZE-1, (matrix_dim-i)/BLOCK_SIZE-1);
	GpuProfiling::prepareProfiling( dimGrid, dimBlock );
      lud_internal<<<dimGrid, dimBlock>>>(m, matrix_dim, i);
	GpuProfiling::addResults("lud_internal");
  }
	GpuProfiling::prepareProfiling( 1,BLOCK_SIZE );
  lud_diagonal<<<1,BLOCK_SIZE>>>(m, matrix_dim, i);
	GpuProfiling::addResults("lud_diagonal");
