...
extern "C" void histogram64(
	...
	GpuProfiling::prepareProfiling( histogramCount, HISTOGRAM64_THREADBLOCK_SIZE );
    histogram64Kernel<<<histogramCount, HISTOGRAM64_THREADBLOCK_SIZE>>>(
        d_PartialHistograms,
        (data_t *)d_Data,
        byteCount / sizeof(data_t)
    );
	GpuProfiling::addResults("histogram64Kernel");
    cutilCheckMsg("histogram64Kernel() execution failed\n");

	GpuProfiling::prepareProfiling( HISTOGRAM64_BIN_COUNT, MERGE_THREADBLOCK_SIZE );
    mergeHistogram64Kernel<<<HISTOGRAM64_BIN_COUNT, MERGE_THREADBLOCK_SIZE>>>(
        d_Histogram,
        d_PartialHistograms,
        histogramCount
    );
	GpuProfiling::addResults("mergeHistogram64Kernel");
