...
main( int argc, char** argv)
{
  ...
  // Start logs
  shrSetLogFileName ("transposeNew.txt");
  shrLog("%s Starting...\n\n", argv[0]);
...
exit(0);
  ...
  for (int k = 0; k<8; k++) {
    ...
    switch (k) {
    case 0:
      kernel = &copy;                           kernelName = "simple copy           "; cleanName = "copy"; break;
    case 1:
      kernel = &copySharedMem;                  kernelName = "shared memory copy    "; cleanName = "copySharedMem"; break;
    case 2:
      kernel = &transposeNaive;                 kernelName = "naive transpose       "; cleanName = "transposeNaive"; break;
    case 3:
      kernel = &transposeCoalesced;             kernelName = "coalesced transpose   "; cleanName = "transposeCoalesced"; break;
    case 4:
      kernel = &transposeNoBankConflicts;       kernelName = "no bank conflict trans"; cleanName = "transposeNoBankConflicts"; break;
    case 5:
      kernel = &transposeCoarseGrained;         kernelName = "coarse-grained        "; cleanName = "transposeCoarseGrained"; break;
    case 6:
      kernel = &transposeFineGrained;           kernelName = "fine-grained          "; cleanName = "transposeFineGrained"; break;
    case 7:
      kernel = &transposeDiagonal;              kernelName = "diagonal transpose    "; cleanName = "transposeDiagonal"; break;
    }
	...
	GpuProfiling::addResults(cleanName);
    ...
    //shrLog("%s\t%5.2f GB/s\t\t%5.2f GB/s\n", kernelName, outerBandwidth, innerBandwidth);

    shrLog("\n");
    shrLogEx(LOGBOTH | MASTER, 0, "transposeNew-Outer-%s, Throughput = %.4f GB/s, Time = %.5f s, Size = %u fp32 elements, NumDevsUsed = %u, Workgroup = %u\n",
           kernelName,
           outerBandwidth,
           outerTime/NUM_REPS,
           (size_x * size_y), 1, TILE_DIM * BLOCK_ROWS);
    shrLogEx(LOGBOTH | MASTER, 0, "transposeNew-Inner-%s, Throughput = %.4f GB/s, Time = %.5f s, Size = %u fp32 elements, NumDevsUsed = %u, Workgroup = %u\n",
           kernelName,
           innerBandwidth,
           innerTime/NUM_REPS,
           (size_x * size_y), 1, TILE_DIM * BLOCK_ROWS);
