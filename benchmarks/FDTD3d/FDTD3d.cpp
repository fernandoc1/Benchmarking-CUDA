...
bool runTest(int argc, const char **argv)
{
    ...
    if (ok)
    {
        ...
        if (shrGetCmdLineArgumentstr(argc, argv, "radius", &dim))
        {
            ...
            {
                shrLogEx(LOGBOTH | ERRORMSG, 0, "radius out of range (%d requested, must be between %d and %d), see header files for details.\n", radius, k_radius_min, k_radius_max);
                ok = false;
            }
        }
        else
        ...
        if (shrGetCmdLineArgumentstr(argc, argv, "timesteps", &dim))
        {
            ...
            {
                shrLogEx(LOGBOTH | ERRORMSG, 0, "timesteps out of range (%d requested, must be between %d and %d), see header files for details.\n", timesteps, k_timesteps_min, k_timesteps_max);
                ok = false;
            }
        }
        else
